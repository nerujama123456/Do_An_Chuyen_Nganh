// lib/screens/appointments/today_appointments_screen.dart
...
class TodayAppointmentsScreen extends StatefulWidget {
  @override
  _TodayAppointmentsScreenState createState() => _TodayAppointmentsScreenState();
}
...
class _TodayAppointmentsScreenState extends State<TodayAppointmentsScreen> {
  ...
  // Định nghĩa chiều rộng cố định cho các cột (Giữ nguyên)
  ...
  void _loadData() async {
    try {
      final appointments = await _dataService.fetchTodayAppointments();

      if (mounted) {
        setState(() {
          _allAppointments = appointments; // Lưu danh sách gốc
  ...
  // HÀM LỌC DỮ LIỆU CỤC BỘ (THEO TẤT CẢ CỘT TRỪ TRẠNG THÁI)
  List<Appointment> _filterAppointments(List<Appointment> appointments, String query) {
    ...
    return appointments.where((app) {
      // Lọc theo Ngày hẹn, Giờ hẹn, Mã BN, Họ tên, Giới tính, Điện thoại, Bác sĩ khám
    ...
    if (mounted) {
      if (success) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Xác nhận lịch hẹn thành công!'), backgroundColor: Colors.green),
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Xác nhận thất bại. Lịch hẹn có thể đã được xác nhận.'), backgroundColor: Colors.red),
        );
      }
      _loadData();
    }
  ...
  Widget build(BuildContext context) {
    const String currentRoute = '/today_appointments';
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  const Text('BỆNH NHÂN KHÁM TRONG NGÀY', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
                  const Divider(),

                  // !!! PHẦN TÌM KIẾM VÀ ĐẶT HẸN MỚI (ĐÃ DI CHUYỂN XUỐNG DƯỚI TIÊU ĐỀ)
                  ...
                  Expanded( // LISTVIEW CUỘN DỌC
                    child: FutureBuilder<List<Appointment>>(
                      ...
                      builder: (context, snapshot) {
                        ...
                        return const Center(child: Text('Không có lịch hẹn trong ngày hôm nay.'));
                      },
                    ),
